...
describe('Parser', function () {
  ...
  it('should translate valid entities into characters', function () {
    assert.equal(Sanitize.sanitize("&apos;&eacute;&amp;"),
      "&apos;&eacute;&amp;");
  });

  it('should translate orphaned ampersands into entities', function () {
    assert.equal(Sanitize.sanitize('at&t'), 'at&amp;t')
  });

  it('should not add newlines after tags when serializing a sanitize', function () {
    assert.equal(
      Sanitize.sanitize("<div>foo\n\n<p>bar</p><div>\nbaz</div></div><div>quux</div>", {elements: ['div', 'p']}),
      "<div>foo\n\n<p>bar</p><div>\nbaz</div></div><div>quux</div>");
  });

  it('should not have the Nokogiri 1.4.2+ unterminated script/style element bug', function () {
    assert.equal(Sanitize.sanitize('foo <script>bar'), 'foo bar');
    assert.equal(Sanitize.sanitize('foo <style>bar'), 'foo bar');
  });

  it('ambiguous non-tag brackets like "1 > 2 and 2 < 1" should be parsed correctly', function () {
    assert.equal(Sanitize.sanitize('1 > 2 and 2 < 1'), '1 &gt; 2 and 2 &lt; 1');
    assert.equal(
      Sanitize.sanitize('OMG HAPPY BIRTHDAY! *<:-D'),
  ...
  it('should work around the libxml2 content-type meta tag bug', function () {
    assert.equal(
      Sanitize.sanitize('<html><head></head><body>Howdy!</body></html>',
    ...
    assert.equal(
      Sanitize.sanitize('<html><head><meta charset="utf-8"></head><body>Howdy!</body></html>', {
        elements: ["html", "head", "meta", "body"],
    ...
    assert.equal(
      Sanitize.sanitize('<html><head><meta http-equiv="Content-Type" content="text/html;charset=utf-8"></head><body>Howdy!</body></html>', {
        elements: ["html", "head", "meta", "body"],
    ...
    // support.
    assert.equal(
      Sanitize.sanitize('<html><head><meta http-equiv="content-type" content="text/html;charset=us-ascii"></head><body>Howdy!</body></html>', {
        elements: ["html", "head", "meta", "body"],
        attributes: {meta: ["charset", "content", "http-equiv"]}
      }),
      "<html><head><meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\"></head><body>Howdy!</body></html>");
