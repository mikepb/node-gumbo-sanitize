...
describe("Malicious HTML", function () {
  ...
  describe("interpolation (ERB, PHP, etc.)", function () {
    it("should escape ERB-style tags", function () {
      assert.equal(
        this.s.sanitize("<% naughty_ruby_code %>"),
        "&lt;% naughty_ruby_code %&gt;");
      assert.equal(
        this.s.sanitize("<%= naughty_ruby_code %>"),
        '&lt;%= naughty_ruby_code %&gt;');
    ...
    it("should remove PHP-style tags", function () {
      assert.equal(this.s.sanitize("<? naughtyPHPCode(); ?>"), "");
      assert.equal(this.s.sanitize("<?= naughtyPHPCode(); ?>"), "");
    });
  });
  ...
  describe("<body>", function () {
    it("should not be possible to inject JS via a malformed event attribute", function () {
      assert.equal(
        this.s.sanitize('<html><head></head><body onload!#$%&()*~+-_.,:;?@[/|\\]^`=alert("XSS")></body></html>'),
  ...
  describe("<iframe>", function () {
    it("should not be possible to inject an iframe using an improperly closed tag", function () {
      assert.equal(
        this.s.sanitize("<iframe src=http://ha.ckers.org/scriptlet.html <"),
        "");
    });
  });
  ...
  describe("<img>", function () {
    it("should not be possible to inject JS via an unquoted <img> src attribute", function () {
      assert.equal(
        this.s.sanitize("<img src=javascript:alert('XSS')>"), "<img>");
    });
    ...
    it("should not be possible to inject <script> via a malformed <img> tag", function () {
      assert.equal(
        this.s.sanitize('<img """><script>alert("XSS")</script>">'),
        '<img>alert(&quot;XSS&quot;)&quot;&gt;');
    });

    it("should not be possible to inject protocol-based JS", function () {
      assert.equal(
        this.s.sanitize('<img src=&#106;&#97;&#118;&#97;&#115;&#99;&#114;&#105;&#112;&#116;&#58;&#97;&#108;&#101;&#114;&#116;&#40;&#39;&#88;&#83;&#83;&#39;&#41;>'),
        "<img>")

      assert.equal(
        this.s.sanitize('<img src=&#0000106&#0000097&#0000118&#0000097&#0000115&#0000099&#0000114&#0000105&#0000112&#0000116&#0000058&#0000097&#0000108&#0000101&#0000114&#0000116&#0000040&#0000039&#0000088&#0000083&#0000083&#0000039&#0000041>'),
        "<img>");

      assert.equal(
        this.s.sanitize('<img src=&#x6A&#x61&#x76&#x61&#x73&#x63&#x72&#x69&#x70&#x74&#x3A&#x61&#x6C&#x65&#x72&#x74&#x28&#x27&#x58&#x53&#x53&#x27&#x29>'),
        "<img>");

      // Encoded tab character.
      assert.equal(
        this.s.sanitize("<img src=\"jav&#x09;ascript:alert('XSS');\">"),
        "<img>");

      // Encoded newline.
      assert.equal(
        this.s.sanitize("<img src=\"jav&#x0A;ascript:alert('XSS');\">"),
        "<img>");
      ...
      // Null byte.
      assert.equal(
        this.s.sanitize('<img src=java\0script:alert("XSS")>'),
        "<img>");
  ...
  describe('<script>', function () {
    it("should not be possible to inject <script> using a malformed non-alphanumeric tag name", function () {
      assert.equal(
        this.s.sanitize("<script/xss src=\"http://ha.ckers.org/xss.js\">alert(1)</script>"),
        "alert(1)");
    });

    it("should not be possible to inject <script> via extraneous open brackets", function () {
      assert.equal(
        this.s.sanitize('<<script>alert("XSS");//<</script>'),
        '&lt;alert(&quot;XSS&quot;);//&lt;');
    });
  });
});
